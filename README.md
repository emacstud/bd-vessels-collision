...
- [Report — findings](#report--findings)
  - [Primary collision candidate — MV Scot Carrier ↔ Karin Høj, 13 Dec 2021](#primary-collision-candidate--mv-scot-carrier--karin-høj-13-dec-2021)
...
By default `--primary-rank 6` is used; this points at the Scot Carrier /
Karin Høj entry in `top_pairs_civilian.json`, which I have verified by
trajectory inspection corresponds to the real collision event. To instead
get the literal minimum-distance pair (which in this dataset is a
fishing-pair formation, see methodology discussion):

```bash
docker compose run --rm vessels-collision --primary-rank 1
```
...
[5/5] visualize (top 10 civilian + top 10 all-ships, one shared Spark session)
... 20 maps rendered ...

======================================================================
PIPELINE COMPLETE
======================================================================
  Primary collision candidate (closest civilian-to-civilian proximity):
...
The rest of `./output/` is gitignored and regenerated by every pipeline run:
...
renders the 10 civilian + 10 all-ships pair maps plus the canonical
collision map in a single shared Spark session.
...
docker compose run --rm vessels-collision python -m src.visualize --rank 6
```
...
### Primary collision candidate — MV Scot Carrier ↔ Karin Høj, 13 Dec 2021

The pipeline identifies the **real-world maritime collision** that occurred 13 December 2021.
...
To recover collisions of large vessels from the candidate set I explicitly
filter operational vessel types (Pilot, Tug, SAR, Law enforcement, Dredging,
Towing, Towing long/wide), as close proximity to other vessels is the usual behaviour of such vessel's types, and inspect the remaining civilian close approaches
with the trajectory visualiser. Additionally, the implementation identifies Top 10 pairs of closest encounters, 
which are the analyzed case by case to filter the actual collision. 
The Karin Høj - Scot Carrier collision trajectory shape is X-crossing 
rather than parallel formation and it is the only pair, in which one of the vessel's 
AIS stopped transmitting after the collision — the distinguishing features visible only after rendering.
...
Rank 6 — bolded — is the answer, selected as the default
`--primary-rank` in `main.py` after manual trajectory inspection.
